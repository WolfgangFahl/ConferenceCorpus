...
class Histogramm(Plot):
    ...
    def prepareHistogram(
        self, facecolor="b", alpha=0.5, density: bool = False, bins=None, label=None
    ):
        self.addHistogramSeries(
            x=self.x,
            facecolor=facecolor,
            alpha=alpha,
            density=density,
    ...
    def addHistogramSeries(
        self, x, facecolor="b", alpha=0.5, density: bool = False, bins=None, label=None
    ):
        if bins is None:
    ...
    def show(
        ...
        self.prepareHistogram(
            facecolor=facecolor, alpha=alpha, density=density, bins=bins, label=label
        )
        plt.xlabel(xLabel)
        plt.ylabel(yLabel)
        if density:
    ...
    def get_image(
        ...
        self.show(
            xLabel, yLabel, title, facecolor, alpha, density, grid, ps, bins, vlineAt
        )
        return ps.outputFile
